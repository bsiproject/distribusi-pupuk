...
class Incomes extends CI_Controller
{
	...
	public function index()
	{
		...
		$orders['Incomes'] = $this->order->get_all_orders($config['per_page'], $page, $search);
		$orders['pagination'] = $this->pagination->create_links();

		$this->load->view('header', $params);
		$this->load->view('income', $orders);
	...
	public function pdf($id)
	{
		if ($this->order->is_order_exist($id)) {
			...
			$html = $this->load->view('pdf/', $params, true);
